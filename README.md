...
teethsee 是带摄像头的智能可视冲牙器演示系统，提供 Wi-Fi 口腔画面接入架构、照片归档、模拟 AI 初筛报告、牙齿长期记录、日历回顾和咨询流程。
...
直接用浏览器打开根目录的 `index.html` 可体验网页演示与本机档案流程。网页只保留 Wi-Fi 摄像头入口，不把模拟状态当作实时画面；型号 `HCSK-352013A-X1-230731` 的真实视频流需要通过 `mobile/` 的 iOS/Android App 接入厂商确认的 RTSP 或私有 UDP 协议。
...
公开网页当前仍是离线演示：AI 报告为确定性的模拟结果。牙位覆盖、照片库、报告事件和提醒设置会通过浏览器 IndexedDB 保存在当前设备；正式账号和跨设备同步仍由 `backend/` 提供。仓库中的 `ai-service/` 已开始独立开发真实离线图片分类，不会改变现有 GitHub Pages 演示地址。
