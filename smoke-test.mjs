...
assert.match(html, /function renderCalendarSummary/, "日历月份统计必须动态计算");
assert.match(html, /eventsForCalendarMonth\(year, month\)/, "月度统计和重要事件必须使用当前月份");
assert.match(html, /id="calendar-important-events"/, "重要事件必须由当前月份事件渲染");
assert.match(html, /tooth === "全口" \? "full-mouth"/, "全口事件必须使用安全稳定的 ASCII 标识");
assert.match(html, /requiredFullMouthEvents/, "旧版档案恢复时必须补回演示清洁事件");
assert.match(html, /window\.history\.pushState/, "页面切换必须写入浏览器历史");
assert.match(html, /window\.history\.replaceState/, "首次加载必须建立可返回的首页历史状态");
assert.match(html, /window\.addEventListener\("popstate"/, "浏览器后退必须恢复应用内页面");
assert.match(html, /switchView\(viewName, \{ historyMode: "none" \}\)/, "恢复历史页面时不得重复写入历史");
...
assert.ok(
  i18nStart > scriptStart && i18nEnd > i18nStart,
  "必须能读取 i18n 翻译核心",
);
...
console.log(`Demo smoke test: PASS (${visibleStrings.size} i18n strings covered)`);
